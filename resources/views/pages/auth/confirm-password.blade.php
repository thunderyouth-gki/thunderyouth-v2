@php
    $isAdmin = Str::contains(session('url.intended'), '/admin/');
@endphp

@if($isAdmin)
    <x-layouts.admin :title="__('Confirm password')">
        <section class="w-full">
            <x-pages::admin.layout heading="Security">
                <div class="max-w-xl">
                    <p class="text-sm text-textlight mb-6">{{ __('This is a secure area of the application. Please confirm your password before continuing.') }}</p>
                    ...
                    <form method="POST" action="{{ route('password.confirm.store') }}" class="flex flex-col gap-6">
                        ...
                        <div>
                            <button type="submit" class="bg-primary hover:bg-primary/90 text-white px-8 py-3 rounded-xl font-bold transition shadow-soft text-sm flex items-center gap-2">
                                <i class="fa-solid fa-shield-halved text-xs"></i> {{ __('Confirm') }}
                            </button>
                        </div>
                    </form>
                </div>
