...
<head>
    ...
    <title>{{ $title ?? 'My Dashboard - Thunder Youth' }}</title>
    <!-- Favicon -->
...
<body class="bg-gradient-to-br from-brandlight/50 to-background dark:from-primary/10 dark:to-background text-text font-sans antialiased h-screen overflow-hidden flex dark:[color-scheme:dark]">
    ...
    <aside class="w-64 bg-surface border-r border-primary/10 dark:border-primary/20 flex-shrink-0 flex flex-col h-full hidden md:flex transition-all duration-300 relative z-20 shadow-sm">
        ...
        <div class="h-16 flex items-center px-6 border-b border-primary/10 dark:border-primary/20">
            <a href="{{ route('dashboard') }}" class="font-bold text-xl flex items-center gap-2 text-primary">
        ...
        <nav class="flex-1 overflow-y-auto py-6 px-4 flex flex-col gap-1">
            ...
            <a href="{{ route('dashboard') }}" class="flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-all {{ request()->routeIs('dashboard') ? 'bg-primary text-white font-bold shadow-md shadow-primary/20' : 'text-textlight hover:bg-primary/10 hover:text-primary' }}">
                <i class="fa-solid fa-gauge w-5 text-center"></i> Dashboard
            </a>
            
            <div class="text-[10px] font-bold text-textlight uppercase tracking-wider mt-6 mb-2 px-2">My Activities</div>
            <a href="{{ route('user.attendances') }}" class="flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-all {{ request()->routeIs('user.attendances') ? 'bg-primary text-white font-bold shadow-md shadow-primary/20' : 'text-textlight hover:bg-primary/10 hover:text-primary' }}">
                <i class="fa-solid fa-clipboard-user w-5 text-center"></i> Attendance History
            </a>
            <a href="{{ route('user.events') }}" class="flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-all {{ request()->routeIs('user.events') ? 'bg-primary text-white font-bold shadow-md shadow-primary/20' : 'text-textlight hover:bg-primary/10 hover:text-primary' }}">
                <i class="fa-solid fa-calendar-days w-5 text-center"></i> Upcoming Events
            </a>
        </nav>
        ...
        <div class="p-4 border-t border-primary/10 dark:border-primary/20 bg-surface mt-auto relative" x-data="{ userMenuOpen: false }" @click.away="userMenuOpen = false">
            ...
            <div x-show="userMenuOpen" 
                ...
                <a href="{{ route('profile.edit') }}" class="block px-4 py-2.5 text-sm font-medium text-text hover:bg-primary/10 hover:text-primary transition">
    ...
    <div class="flex-1 flex flex-col h-full overflow-hidden relative">
        ...
        <header class="h-16 bg-surface/80 backdrop-blur-md border-b border-primary/10 dark:border-primary/20 flex items-center justify-between px-4 sm:px-6 lg:px-8 flex-shrink-0 shadow-sm relative z-10">
            <div class="flex items-center">
                ...
                <a href="{{ route('dashboard') }}" class="md:hidden font-bold text-lg flex items-center gap-2 text-primary mr-4">
                    <img src="{{ asset('storage/thunder-logo.png') }}" alt="Thunder Youth Logo" class="w-8 h-8 object-contain">
                </a>
